...
менеджеру в Telegram карточку с именем/мессенджером/видом страховки/языком/
вопросом (если настроены `TELEGRAM_BOT_TOKEN` + `TELEGRAM_MANAGER_CHAT_ID`).
WhatsApp/Viber — без сбора контакта (deep-link с предзаполненным текстом);
Telegram-ник планируется дозахватывать через вебхук бота (следующий шаг).
