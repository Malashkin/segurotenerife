...
**Захват Telegram-ника через бота.** Для канала Telegram клиент уходит в бота
(`t.me/<bot>?start=<lead_id>`), а не в личку менеджера. По нажатию Start вебхук
`POST /api/telegram/webhook` захватывает `@ник` клиента, дозаписывает его в
`leads.contact` (идемпотентно) и шлёт получателям карточку лида уже **с ником**,
а клиенту отвечает, что менеджер скоро напишет (4 языка). `lead_id` генерится на
клиенте (синхронная ссылка без блокировки попапа), бэкенд сохраняет лид с этим
id. Вебхук защищён `X-Telegram-Bot-Api-Secret-Token` (`TELEGRAM_WEBHOOK_SECRET`).
Схема всех каналов — `docs/architecture/lead-flow.md`.
