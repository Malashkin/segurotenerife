/**
 * Локальная декларация переменных окружения Vite для библиотеки widgets.
 *
 * widgets импортирует ChatWidget из @features (через ChatLauncher), который
 * транзитивно тянет chat/model/handoff.ts и shared/api/config.ts — оба читают
 * import.meta.env. Сама библиотека не зависит от пакета `vite` (code-weight.md),
 * поэтому объявляем нужную часть типа здесь, чтобы strict-typecheck проходил.
 *
