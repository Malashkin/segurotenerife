...
**Статус:** реализовано. Приложение — `frontend/apps/web-astro` (`@seguro/web-astro`).
Прежний Vite-SPA `frontend/apps/web` пока сохранён (паритет подтверждён), но
**целевой web — Astro**; SPA убираем после полного переключения e2e/деплоя.
...
## Осталось для полного перехода (бэклог)
1. Перевести Playwright e2e на Astro-сборку (сейчас suite ещё на старом `apps/web`,
   16/16 зелёные) и адаптировать селекторы (чат открывается через `data-ui`,
   язык — навигацией по `/es/` и т.п.).
2. Переключить деплой на `apps/web-astro/dist`; на статик-хосте убедиться, что
   `/es/ /uk/ /en/` отдаются как свои файлы (Astro генерит подпапки — обычно ок).
3. После стабилизации — удалить `apps/web` и связанные костыли
   (`scripts/seo-prerender.mjs`, URL-логику локалей в `detect.ts`).
