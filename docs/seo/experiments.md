...
> ⚠️ **Отсчёт не начат (проверено 2026-09-08, SEGU-13).** Все правки от
> 2026-09-05 лежат в репозитории, но на продакшене их нет: живые title у
> `funeral-insurance-spain`, `pet-insurance-spain` и `digital-nomad-visa-insurance`
> — старые, статья `travel-insurance-tenerife` отдаёт soft-404. Позиции по
> четырём чистым замерам H1 за неделю не сдвинулись ни на пункт, что это
> подтверждает. **Дата 2026-10-05 недействительна.** Новая дата вердикта =
> день деплоя + 14; вписать сюда, когда деплой пройдёт:
> `деплой состоялся ГГГГ-ММ-ДД → вердикт не раньше ГГГГ-ММ-ДД`.
