...
| Что | Когда | Чем запускается | Состояние на 2026-09-18 |
|---|---|---|---|
| Контент-рутина (`/seo-daily-article`) | пн / ср / пт, 09:00 Atlantic/Canary | автопилот `SEO · контент-рутина` (Dácil) | **заводится, SEGU-23** |
| Недельный разбор (`/seo-weekly`) | понедельник, 08:00 UTC | автопилот `SEO-неделя` (Yeray) | работает с 2026-09-08 |
| Живой диалог с чатом | ежедневно, 09:00 UTC | автопилот `Живой диалог с чатом` (Yeray) | работает с 2026-09-09 |
| Месячное планирование (`/seo-monthly`) | 1-е число | — | **не заведено**, решение отложено до 2026-10-01 |
...
**Чего в этом расписании нет: ежедневного `healthcheck.py`.** Refresh-токен Search
Console живёт 7 дней, пока OAuth-приложение в статусе Testing, и умирает молча;
при недельной частоте проверки данные могут быть слепыми до семи суток. Лечится не
расписанием, а публикацией приложения — это доступы, то есть владелец
(см. «Кого именно звать»).
