...
- **Карточки** секции «Статьи» кликабельны там, где статья для локали есть;
  иначе «Скоро».
- Опубликовано: **24 темы × 4 языка (ru/uk/en/es) = 96 статей**. Последними
  добавлены 5 тем по продуктовым линиям базы знаний: полис с возмещением
  (reembolso), что такое cuadro médico, международный медполис, ритуальное
  страхование (decesos), страховка для приезжающих в Испанию (travel/welcome);
  перед этим — воссоединение семьи, первые шаги после переезда, нелюкративная
  виза (NLV), частная vs государственная медицина, студенческая виза, как
  выбрать/сменить страховку, стоматология, питомцы, пенсионеры, роды.
